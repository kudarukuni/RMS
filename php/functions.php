...
class login_registration_class{
	...
	public function updatePassword($sid, $newpass, $oldpass){
		...
		if($count == 0){
			return print("<p style='color:red;text-align:center'>Incorrect Old Password.</p>");
		}else{
			$update = $conn->query("update st_info set password='$newpass' where st_id='$sid' ");
			return print("<p style='color:green;text-align:center'>Successfully Changed Password.</p>");
		}
	}
